...
void TestCase::doWork()
{
	...
	for (int r = 0; r < painted.rows; r++)
	{
		int start = -1;
		for (int c = 0; c < painted.cols; c++)
		{
			// find end
			if (start != -1)
			{
				if(wall.at<uchar>(r, c) != 255)
				{
					int end = c - 1;
					stringstream ss;
					ss << "PAINT_LINE " << r << " " << start << " " << r << " " << end;
					result.push_back(ss.str());
					line(painted, Point2i(start, r), Point2i(end, r), Scalar::all(255));
					start = -1;
				}
			}

				// find start
			else if(wall.at<uchar>(r, c) == 255)
			{
				start = c;
			}
		}
